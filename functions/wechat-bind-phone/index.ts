...
Deno.serve(async (req: any) => {
  ...
  try {
    ...
    const { openid, unionid, phoneNumber, encryptedData, iv, code, userInfo } = await req.json();
    ...
    if (encryptedData && iv && code) {
      ...
      // Note: This is a simplified version. In production, you should use a proper crypto library
      // to decrypt the phone number using AES-128-CBC with session_key and iv
      // For now, we'll require phoneNumber to be passed directly or use a proper decryption library
    ...
    const { error: profileError } = await supabase
      ...
      .upsert({
        ...
        wechat_openid: openid,
        wechat_unionid: unionid || user.user_metadata?.wechat_unionid || null, // Sync unionid to profiles table
        phone: finalPhoneNumber, // Sync phone number to profiles table
      }, {
        onConflict: 'id'
      });
